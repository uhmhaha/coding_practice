...
public class ScovilleEfficiency2 {
    public static int solution(int[] scoville, int K) {
        ...
        while(isOver) {
        	...
        	if(scovilleArr.get(0) < K) {
            	...
            	//sorting logic recreate
            	//Collections.sort(scovilleArr);
        		
        		for(int i = 1; i < scovilleArr.size(); i++ ) {
        			if( scovilleArr.get(i) >= scovilleArr.get(0)) {
        				scovilleArr.add(i, scovilleArr.get(0));
        				scovilleArr.remove(0);
        				break;
        			}
        		}
